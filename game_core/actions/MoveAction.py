...
class MoveAction(Action):
    ...
    def describe(self):
        return f"Unit {self.unit} moved from {self.origin} to {self.destination.name}"

    def validate(self):
        if self.unit.engaged and self.origin != self.destination:
            raise ValueError(f"[MoveAction#{self.id}:validate] Unit is engaged and cannot move")
        
        if self.destination not in self.origin.adjacent and self.destination != self.origin:
            raise ValueError(f"[MoveAction#{self.id}:validate] Destination is not adjacent to origin")

        for unit in self.destination.units:
            if unit.zone == self.destination and unit.owner != self.unit.owner:
                raise ValueError(f"[MoveAction#{self.id}:validate] Enemy units present in destination")
            
        if self.unit.owner.faction == Faction.CARTHAGE:
            if self.origin.is_city and not self.destination.is_city:
                raise ValueError(f"[MoveAction#{self.id}:validate] Carthage cannot leave the city")
            
        if self.origin.id == "north_wall" and self.destination.id == "lagoon":
            raise ValueError("[MoveAction#{self.id}:validate] Cannot move from North Wall to Lagoon")
